...
namespace WeifenLuo.WinFormsUI.Docking
{
	...
	public class DockContentHandler : IDisposable, IDockDragSource
	{
        ...
        // !!! danny 20090112 - S
        public void Show(DockPanel dockPanel, DockState dockState, Rectangle FloatWindowsBounds)
        {
            if (dockPanel == null)
                throw (new ArgumentNullException(Strings.DockContentHandler_Show_NullDockPanel));
            ...
            {
                // !!! danny 20090429 - S
                DockPanel.DefaultFloatWindowLocation = new Point(FloatWindowsBounds.X, FloatWindowsBounds.Y);
                // !!! danny 20090429 - E
                Pane = DockPanel.DockPaneFactory.CreateDockPane(Content, DockState.Float, true);
                Pane.FloatWindow.Bounds = FloatWindowsBounds;
            }
            else if (PanelPane == null)
            {
                DockPane paneExisting = null;
