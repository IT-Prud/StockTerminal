...
namespace TradeDB.Net
{
    public partial class QuoteConnector : Processor
    {
        ...
        public new enum ProcessState { Stopped, SessionLogin, Connecting, ServerLogin, Ready, Disconnecting, MaxState };
        ...
        public MsgArrivalDelegate MsgArrival;
        ...
        private int MaxLoginAttemptCount = -1; // < 0 will keep retry indefinitely
        ...
        public SpreadTableSet SpreadTables = null;
        private int TimeLastGetSpreadTable = ProcessTimer.TickCount - 10000000;
        private int TimeToGetSpreadTable = 5000;
        ...
        private byte[] DcpBuffer = new byte[100000];
        ...
        public string LogFolder
        {
            ...
            set
            {
                ...
                if (pLogFolder != null)
                {
                    DataInLog = Program.TheLogProcessor.Open("SocketQuoteInLog", Path.Combine(pLogFolder, @"__SocketLog\Quote\"), "In", ".log");
                    DataInLog.FlushAfterAppend = true;
                    DataOutLog = Program.TheLogProcessor.Open("SocketQuoteOutLog", Path.Combine(pLogFolder, @"__SocketLog\Quote\"), "Out", ".log");
        ...
        public void Send(TradeMessage Message)
        {
            if (Message != null)
            {
                string msgSeqNumStr;
                string errorMessage;
                MessageRaw msgRaw = Message.GetRaw(MsgRawPool);
        ...
        private void ProcessSessionLogin(bool StateChanged, long StateChangedLastTime, ref int NextState, ref int NextSubState, ref long WaitTime)
        {
            ...
            else if (LoginAttemptCount < MaxLoginAttemptCount || MaxLoginAttemptCount < 0)
            {
                ...
                if (pUserId == "" || GetPasswordSalt() == 0)
                {
                    switch (SessionLogin())
                    {
                        ...
                        case -5000: // Invalid VerificationCode
        ...
        private void ProcessServerLogin(bool StateChanged, long StateChangedLastTime, ref int NextState, ref int NextSubState, ref long WaitTime)
        {
            switch (ServerLoginState)
            {
                ...
                case 1:
                    ProcessReceived();
                    if (ProcessSessionMessage(NextState))
                    {
                        WaitTime = 0;
                    }
        ...
        private void ProcessReady(bool StateChanged, long StateChangedLastTime, ref int NextState, ref int NextSubState, ref long WaitTime)
        {
            ...
            if (pLastError != LastErrorEnum.ForceLogout && TcpSock.IsConnected)
            {
                if (SpreadTables.Count <= 0)
                {
                    ...
                    if ((nowTime - TimeLastGetSpreadTable) >= TimeToGetSpreadTable)
                    {
                        GetSpreadTable();
                        TimeLastGetSpreadTable = nowTime;
                        TimeToGetSpreadTable = Math.Min(600000, TimeToGetSpreadTable << 1);
                    }
                }
