...
namespace TradeDB.Net
{
    public partial class TradeConnector : Processor
    {
        ...
        private bool ProcessReceived(out string ErrorMessage)
        {
            MessageRaw msgRaw;
            int count;
            ...
            do
            {
                ...
                {
                    ReceiveBuffer.Append(msgRaw.Data, msgRaw.Count, false);
                    msgRaw.Recycle();

                    count = ReceiveBuffer.SpaceOccupied & -16;

                    try
                    {
                        Decryptor.TransformBlock(ReceiveBuffer.UnderBuffer, 0, count, MsgBuffer.UnderBuffer, MsgBuffer.SpaceOccupied);
                    }
                    catch (Exception exp)
                    {
                    ...
                    ReceiveBuffer.BytesRead = count;
                    MsgBuffer.BytesAppended = count;
                ...
                {
                    startIndex = 0;
                    msgList = TradeMessage.Parse(MsgBuffer.UnderBuffer, ref startIndex, MsgBuffer.SpaceOccupied, false);
                        ...
                        for (int i = 0; i < msgList.Count; i++)
                        {
                            ...
                            switch (msg.MessageId)
                            {
                                case "ZIP":
                                    ...
                                    startIndex = 0;
                                    msgDecompList = TradeMessage.Parse(DcpBuffer, ref startIndex, dataLength, false);
